...
<template>
  ...
  >
    <div v-if="leadOpen"
         class="fixed inset-0 z-[60] flex items-start sm:items-center justify-center overflow-y-auto bg-slate-900/60 backdrop-blur-sm p-4 sm:p-6"
         @click.self="closeLead">
      <div class="relative w-full max-w-lg my-8">
        <button @click="closeLead" aria-label="Закрыть"
                class="absolute -top-3 -right-3 z-10 flex h-10 w-10 items-center justify-center rounded-full bg-white border border-line text-cloud hover:text-gold hover:border-gold transition-colors shadow-md">
        ...
        <div class="text-center mb-5">
          <h3 class="font-display font-bold text-2xl md:text-3xl text-white">Оставить заявку</h3>
          <p class="mt-2 text-slate-300">Перезвоним и подберём авто под ваш бюджет.</p>
        </div>
        <LeadFormCard />
      </div>
    </div>
